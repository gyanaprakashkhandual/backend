...
router.get("/category/:category", (req, res) => {
    try {
        ...
        const categoryPath = path.join(__dirname, `../../public/skills/${category}`);
...
router.get("/category/:category/:subcategory", (req, res) => {
    try {
        ...
        const subcategoryPath = path.join(__dirname, `../../public/skills/${category}/${subcategory}`);
...
router.get("/:skillName", (req, res) => {
    try {
        ...
        const allSkills = readSkillsFromDir(skillsDir);
        const foundSkill = allSkills.find((s) => s.skillName.toLowerCase() === skillName.toLowerCase());
        if (!foundSkill) {
            return res.status(404).json({
                success: false,
