...
router.get("/", (req, res) => {
    try {
        const filePath = path.join(__dirname, "../../public/experience/experience.json");
...
router.get("/company/:company", (req, res) => {
    try {
        ...
        const data = fs.readFileSync(filePath, "utf-8");
        const experienceData = JSON.parse(data);
        const company = req.params.commpany;
        const filteredExperience = experienceData.experience.filter((exp) => exp.company.toLowerCase() === company.toLowerCase());
...
router.get("/role/:role", (req, res) => {
    try {
        ...
        const filteredExperience = experienceData.experience.filter((exp) => exp.role.toLowerCase().includes(role.toLowerCase()));
        if (filteredExperience.length === 0) {
            return res.status(404).json({
                success: false,
...
router.get("/:slug", (req, res) => {
    try {
        ...
        const foundExperience = experienceData.experience.find((exp) => exp.slug === slug);
        if (!foundExperience) {
            return res.status(404).json({
                success: false,
