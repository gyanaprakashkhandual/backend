...
router.get("/", (req, res) => {
    try {
        const filePath = path.join(__dirname, "../../public/education/education.json");
...
router.get("/stream/:stream", (req, res) => {
    try {
        ...
        const filteredEducation = educationData.education.filter((edu) => edu.stream.toLowerCase() === stream.toLowerCase());
...
router.get("/institution/:institution", (req, res) => {
    try {
        const filePath = path.join(__dirname, "../../public/education/education.json");
        const data = fs.readFileSync(filePath, "utf-8");
        const educationData = JSON.parse(data);
        const institution = req.params.institution;
        const filteredEducation = educationData.education.filter((edu) => edu.institution.toLowerCase() === institution.toLowerCase());
        if (filteredEducation.length === 0) {
            return res.status(404).json({
                success: false,
...
router.get("/:title", (req, res) => {
    try {
        ...
        const foundEducation = educationData.education.find((edu) => edu.title.toLowerCase() === title.toLowerCase());
        if (!foundEducation) {
            return res.status(404).json({
                success: false,
